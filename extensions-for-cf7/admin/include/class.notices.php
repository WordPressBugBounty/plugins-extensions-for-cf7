...
if ( ! class_exists( 'Extensions_Cf7_Notice' ) ){
    class Extensions_Cf7_Notice{
        ...
        public function __construct(){
            ...
            add_action( 'extcf7_admin_notices', [ $this, 'show_admin_notices' ] );
            add_action( 'extcf7_admin_sidebar_notices', [ $this, 'show_admin_sidebar_notices' ] );
            add_action(	'admin_footer', [ $this, 'enqueue_scripts' ], 999 );
            add_action( 'wp_ajax_extcf7_notices', [ $this, 'ajax_dismiss' ] );
        }
        ...
        public function ajax_dismiss() {
			...
			if ( ! apply_filters( 'extcf7_notice_user_cap_check', current_user_can( $capability ) ) ) {
                $error_message = [
                    'message'  => __('You are not authorized.', 'cf7-extensions')
                ];
                wp_send_json_error( $error_message );
			}

            // Nonce verification check
            if( !wp_verify_nonce( $nonce, 'extcf7_notices_nonce') ) {
                $error_message = [
                    'message'  => __('Are you cheating?', 'cf7-extensions')
                ];
                wp_send_json_error( $error_message );
            }

            if ( ! empty( $notice_id ) && (strpos( $notice_id, 'extcf7-notice' ) !== false) ) {
        ...
        public function enqueue_scripts() {
            ...
            .extcf7-admin-notice.notice img, .extcf7-review-notice-wrap img{
                width: 100%;
            }
            .extcf7-review-notice-wrap{
            ...
            .extcf7-review-notice-content {
                margin-left: 15px;
            }
            .extcf7-review-notice-action {
                display: flex;
                align-items: center;
                padding-top: 10px;
                gap: 15px;
            }
            .extcf7-review-notice-action span.dashicons {
                font-size: 1.4em;
                padding-left: 0;
            }
            .extcf7-review-notice-action a {
                padding-left: 0;
                text-decoration: none;
                display: flex;
                gap: 5px;
            }
            .extcf7-review-notice-content h3 {
                margin: 0;
            }";

            $scripts = "jQuery(document).ready( function($) {
                $( '.extcf7-admin-notice.is-dismissible' ).on( 'click', '.notice-dismiss,.extcf7-notice-close', function(e) {
                    e.preventDefault();
                    let noticeWrap = $( this ).parents( '.extcf7-admin-notice' ),
                        ...
                        noticeNonce = '".esc_html( wp_create_nonce( 'extcf7_notices_nonce' ) )."';
        ...
        private function prepare_notice( $notice_data ){
            ...
            if ( isset( $notice['type'] ) ) {
                ...
                if( $notice['type'] !== 'custom'){
                    ...
                    $notice['dismissible_btn'] = '<button type="button" class="notice-dismiss"><span class="screen-reader-text">'.esc_html__('Dismiss this notice.','cf7-extensions').'</span></button>';
                }
            }
            ...
            $notice['id'] = 'extcf7-notice-id-' . $notice['id'];
    ...
    // Call instance
    Extensions_Cf7_Notice::instance();
