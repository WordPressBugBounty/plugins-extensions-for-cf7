...
if ( ! class_exists( 'Extensions_Cf7_Diagnostic_Data' ) ) {
    class Extensions_Cf7_Diagnostic_Data {
        ...
        private function __construct() {
            ...
            if ( get_option( 'ht_cf7extensions_diagnostic_data_agreed' ) === 'yes' || get_option( 'ht_cf7extensions_diagnostic_data_notice' ) === 'no' ) {
                return;
            }
        ...
        public function should_show_notice() {
            ...
            foreach ( $sibling_plugins as $plugin_slug => $options ) {
                if ( get_option( $options['agreed'] ) === 'yes' ) {
                    update_option( 'ht_cf7extensions_diagnostic_data_agreed', 'yes' );
                    update_option( 'ht_cf7extensions_diagnostic_data_notice', 'no' );
                    return false;
                }
            }
            ...
            if ( isset( $ht_diagnostic_notice_owner ) && $ht_diagnostic_notice_owner !== 'ht_cf7extensions' ) {
                return false;
            }
            $ht_diagnostic_notice_owner = 'ht_cf7extensions';
        ...
        private function show_core_notice() {
            $message_l2 = sprintf( esc_html__( 'Server information (Web server, PHP version, MySQL version), WordPress information, site name, site URL, number of plugins, number of users, your name, and email address. You can rest assured that no sensitive data will be collected or tracked. %1$sPrivacy Policy%2$s', 'cf7-extensions' ), '<a target="_blank" href="' . esc_url( $this->privacy_policy ) . '">', '</a>' );
            ...
            ?>
            <div class="extcf7-admin-notice notice notice-info ht_cf7extensions-diagnostic-data-notice">
                <p class="ht_cf7extensions-diagnostic-data-message"><?php echo wp_kses_post( sprintf( esc_html__( 'Want to help make %2$s%1$s%3$s even more awesome? Allow %1$s to collect diagnostic data and usage information. (%4$swhat we collect%5$s)', 'cf7-extensions' ), esc_html( $this->project_name ), '<strong>', '</strong>', '<a href="#" class="ht_cf7extensions-diagnostic-data-list-toogle">', '</a>' ) ); ?></p>
